...
## Boreas project overrides

Upstream content above is unmodified (imported from mindrally/skills). Two points conflict with this project's rules and are overridden here — project rules win:

- **Package name:** ignore "import from motion/react" — this project has `framer-motion@^12.38.0` installed and every existing component imports from `"framer-motion"`. Keep using that import path; do not migrate to the `motion` package.
- **Springs:** ignore "prefer spring animations." GUIDELINES/DESIGN anti-bans forbid bounce/elastic motion. Use duration + the project's standard ease-out exponential curve (`[0.22, 1, 0.36, 1]`, already defined as `EASE` in `components/hero/boreas-hero.tsx`) instead of `type: "spring"`, unless a specific gesture (e.g. drag-release) needs spring physics — and even then, damping must be high enough to fully suppress overshoot.
