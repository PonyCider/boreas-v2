...
4. **Elementos decorativos — ilustrativos (repartidos a lo largo de todo el
   recorrido, no en un solo punto):** `AccentOrbField` y `DrawnPathAccent` (ver Sección
...
4. **Debajo de la card pineada, con scroll adicional:** salen los elementos decorativos
   ...
   `DrawnPathAccent` a menor escala (mobile usa menos orbes / línea más corta que
...
Ambos primitivos van en `components/motion/`, mismo patrón de export/props que
...
- `prefers-reduced-motion` (`HeroStatic` y su rama mobile): sin intro de letras, sin
  ...
  coreografía. `AccentOrbField`/`DrawnPathAccent` en modo reducido: estáticos, sin
...
- El wordmark y headline deben seguir siendo texto real (no imágenes/canvas) —
  ...
  se revela).
- `AccentOrbField`/`DrawnPathAccent`: `aria-hidden="true"`, `pointer-events-none` — cero
...
- Desktop, scroll continuo: texto se acomoda a la izquierda, aparece párrafo+CTA, luego
  la card, luego los 4 proof-points como chips uno por uno, orbes y línea decorativa
...
- Mobile: mismo intro; scroll hace desaparecer Boreas+H1 y aparecer la card; la card se
  pinea arriba al llegar; más abajo aparecen los chips + orbes/línea a menor escala.
