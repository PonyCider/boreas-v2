...
- **Contexto operativo en cada repo de cliente:** una versión condensada de este documento
  ...
  `boreas-template`, y debe copiarse manualmente a cada `boreas-<cliente>` clonado de él — el
  "Template repository" de GitHub no propaga contenido gitignoreado (ver nota en el README de
  `boreas-template`). Es una copia derivada, no una segunda fuente de verdad — si hay
  discrepancia, gana este documento.
